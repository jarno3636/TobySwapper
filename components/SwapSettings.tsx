...
export default function SwapSettings({ open, onClose, slippagePct, setSlippagePct }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* panel */}
      <div
        className="relative w-full max-w-sm rounded-2xl border-2 border-black p-5 text-slate-100
                   bg-[linear-gradient(180deg,rgba(10,16,28,.96),rgba(5,10,20,.96))]
                   shadow-[0_12px_0_#000,0_24px_54px_rgba(0,0,0,.65)]"
      >
        <div className="mb-3 flex items-center justify-between">
          <div className="font-black text-lg">Settings</div>
          <button
            className="inline-grid h-8 w-8 place-items-center rounded-full border-2 border-black bg-[linear-gradient(180deg,#0f172a,#121826)] text-slate-100 shadow-[0_4px_0_#000]"
            onClick={onClose}
            aria-label="Close"
            title="Close"
          >
